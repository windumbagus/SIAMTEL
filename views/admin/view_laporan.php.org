...
<!-- 		   	<link rel="stylesheet" type="text/css" href="assets/dataTable/DataTables-1.10.16/css/jquery.dataTables.min.css">
 ...
 -->
	<div> 
		   ...
		   $username = $_GET['id'];
		   $query_mysql = mysql_query("SELECT * FROM user WHERE username='$username'")or die(mysql_error());
		   while($user = mysql_fetch_array($query_mysql)){
			?>
	...
	<p>Rekap Absensi :</p>
		...
		<tbody>
			<?php
				require_once("config/koneksi.php");
				$sql = mysql_query(" SELECT * FROM master INNER JOIN jurnal ON master.username=jurnal.username AND master.tanggal=jurnal.tanggal WHERE master.username='$username' order by master.tanggal desc ");
	...
	<div>
		...
		<p>Penilaian :</p>
		 	<?php 
			   require_once("config/koneksi.php");
			   $username = $_GET['id'];
			   $query_mysql = mysql_query("SELECT * FROM nilai WHERE username='$username' ")or die(mysql_error());
